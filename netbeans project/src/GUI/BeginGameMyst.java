...
public class BeginGameMyst extends JFrame {
	 ...
	 * @throws FILException 
	 */
	public BeginGameMyst() throws API.FILException {
		...
		try {
			...
			if(multiplayeroptiongui) {
				while (MysteryModeOptions.getTimeMusic() > (34360748*count1)) {
					count++;
					count1++;
					count2 = count;
					count3 = count1;
				}
				clip.setMicrosecondPosition(MysteryModeOptions.getTimeMusic() - (34360748*count));
				multiplayeroptiongui = false;
			}else {
				while (BeginGameMyst.getTimeMusic() > (34360748*count3)) {
					count2++;
					count3++;
					count = count2;
					count1 = count3;
				}
				clip.setMicrosecondPosition(BeginGameMyst.getTimeMusic() - (34360748*count2));
			}
		...
		boolean multiplatop = MysteryModeOptions.getBool();
		
		if(multiplatop) {
			Point loc = MysteryModeOptions.getLoca();
			setBounds(loc.x, loc.y, widthfile, heightfile);
			MysteryModeOptions.setBool(false);
		}else {
			Point loc = BeginGameMyst.getLoca();
			setBounds(loc.x, loc.y, widthfile, heightfile);
		}
	...
	public void chekagens() {
		if (game.myBord.mysteryWinnercheck(game.activePlayer, linha)) {
			String jogadore = "";
			...
			if(fram.run(widthfile, heightfile)) {
				try {
					...
					clip.stop();
					BeginGameMyst frm = new BeginGameMyst();
					frm.setVisible(true);
				...
				clipTimePostion = clip.getMicrosecondPosition();
				clip.stop();
				MysteryModeOptions frm = new MysteryModeOptions();
				frm.setVisible(true);
